...
Ext4文件系统是由众多块组构成。为了减少由于碎片造成的性能困难，块分配器会非常努力地将每个文件的块保留在同一组中，从而减少寻道时间。块组的大小在 ```sb.s_blocks_per_group``` 中指定，也可以由 ```8 * block_size_in_bytes```算出来。块大小默认为4KB，每个块组包含了32768个块，所以大小为128MB。块组的数量可以由存储设备的大小除以当个块组大小得出来。
